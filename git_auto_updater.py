...
def exec_commands(*args: str) -> str or None:
    results: list[str] = []
    for command in args:
...
def process():
    while True:
        try:
            ...
            if updated:
                if running_context.update_action is UpdateAction.RESTART:
                    terminate_and_wait()
                    running_context.pipe = None
                if running_context.update_action is UpdateAction.START_NEW:
                    running_context.pipe = None
            if running_context.pipe is None:
                running_context.pipe = Popen(shell_split(running_context.startup_command),
                                             cwd=running_context.soft_path)
...
def main():
    ...
    parser.add_argument('-u', '--update-action', default='none', choices=['none', 'restart', 'start-new'],
                        help='action to be done after new version is loaded ' +
                             '(none - do nothing, restart - terminate previous instance and run new, start-new - ' +
                             'starts new instance without terminating previous)')
